...
// Ensure the response is not null before performing assertions
    ...
    void getSolutions_ValidIds_SolutionsReturned() {
        ...
        List<ChallengeSolutionDto.SolutionDto> solutionDtos = Arrays.asList(
                new ChallengeSolutionDto.SolutionDto(UUID.randomUUID(), "Solution Text 1"),
                new ChallengeSolutionDto.SolutionDto(UUID.randomUUID(), "Solution Text 2")
        );
        ChallengeSolutionDto challengeSolutionDto = new ChallengeSolutionDto(UUID.randomUUID(), UUID.randomUUID(), solutionDtos);
        GenericResultDto<ChallengeSolutionDto> expectedResult = new GenericResultDto<>();
        expectedResult.setInfo(offset, limit, 1, new ChallengeSolutionDto[]{challengeSolutionDto});

        when(challengeService.getSolutions(idChallenge, idLanguage, offset, limit)).thenReturn(Mono.just(expectedResult));

        webTestClient.get()
                .uri("/itachallenge/api/v1/challenge/solution/challenge/{idChallenge}/language/{idLanguage}?offset={offset}&limit={limit}", idChallenge, idLanguage, offset, limit)
                .exchange()
                .expectStatus().isOk()
                .expectBody(new ParameterizedTypeReference<GenericResultDto<ChallengeSolutionDto>>() {})
                .value(dto -> {
                    assert dto != null;
