...
/**
 * ✅ ChallengeControllerHappyPathTest
 * This suite focuses exclusively on **happy-path** request flows for ChallengeController.
 * It uses @WebFluxTest for lightweight reactive tests with mocked service dependencies.
 * Exception-handling, invalid inputs, and error responses are covered separately in the
 * MVC-based error-handling test suite.
 */
@WebFluxTest(controllers = ChallengeController.class)
@ExtendWith(SpringExtension.class)
@ActiveProfiles("test")
class ChallengeControllerHappyPathTest {
    ...
    void getAllChallenges_ValidPageParameters_ChallengesReturned() {
        ...
        when(challengeService.getAllChallenges(0, 3))
    ...
    void getAllChallenges_NullPageParameters_ChallengesReturned() {
        ...
        when(challengeService.getAllChallenges(0, 2))
                .thenReturn(Mono.just(expectedResult));
    ...
    void getSolutions_ValidIds_SolutionsReturned() {
        ...
        when(challengeService.getSolutions(idChallenge, idLanguage))
                .thenReturn(Mono.just(expectedResult));

        webTestClient.get()
                .uri("/itachallenge/api/v1/challenge/solution/challenge/{idChallenge}/language/{idLanguage}",
                        idChallenge, idLanguage)
                .exchange()
                .expectStatus().isOk()
    ...
    void getRelatedChallenges_ValidId_Returns200_RelatedChallengesReturned() {
        ...
        webTestClient.get()
                ...
                .consumeWith(response -> {
                    ...
                    Assertions.assertEquals(3, body.getCount());
                });
    }
    ...
    void addChallenge_test_validRequest() {
        ...
        ChallengeCreateDto formDataLocal = new ChallengeCreateDto("títol", "descripció",
    ...
    void addChallengeToBookmarks_Success_Returns200() {
        String existingChallengeId = "existing_challengeId";
        String userId = "existing_userId";
        String authHeader = "validAuthHeader";

        BookmarkDto expectedResponse = new BookmarkDto(true, 20);

        when(challengeService.addChallengeToBookmarks(existingChallengeId, userId))
                .thenReturn(Mono.just(expectedResponse));
        when(challengeJwtFacade.getUserUuIdFromAuthenticationHeader(authHeader))
                .thenReturn(userId);

        webTestClient.post()
                .uri("/itachallenge/api/v1/challenge/challenges/" + existingChallengeId + "/bookmarks")
                .header("Authorization", authHeader)
                .exchange()
                .expectStatus().isOk()
        ...
        verify(challengeService, times(1)).addChallengeToBookmarks(existingChallengeId, userId);
    }
    ...
    void getChallengesByFilter_shouldReturnOkResponse() {
        ...
        UUID languageMock = UUID.randomUUID();

        when(challengeService.getChallengesByFilter(
                Optional.of(languageMock.toString()),
                Optional.of("EASY"),
        ...
        webTestClient.get()
                .uri(uriBuilder -> uriBuilder
                        .path("/itachallenge/api/v1/challenge/challenges/byFilter")
                        .queryParam("idLanguage", languageMock.toString())
    ...
    void removeChallengeFromBookmarks_Success_Returns200() {
        ...
        when(challengeService.removeChallengeFromBookmarks(existingChallengeId, userId))
                .thenReturn(Mono.just(expectedResponse));
        when(challengeJwtFacade.getUserUuIdFromAuthenticationHeader(authHeader))
                .thenReturn(userId);
        ...
        verify(challengeService, times(1))
                .removeChallengeFromBookmarks(existingChallengeId, userId);
    }
