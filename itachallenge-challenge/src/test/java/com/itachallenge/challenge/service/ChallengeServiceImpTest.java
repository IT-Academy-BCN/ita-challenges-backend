...
//    }
    ...
    void testGetSolutionsFromChallenge() {
        ...
        List<SolutionDto> expectedSolutionsDto = List.of(solutionDto1, solutionDto2);
        List<TrimmedSolutionDto> expectedTrimmedSolutionsDto = List.of(trimmedSolutionDto1, trimmedSolutionDto2);
        ...
        when(solutionConverter.convertDocumentFluxToDtoFlux(any(), any())).thenReturn(Flux.fromIterable(expectedSolutionsDto));
        when(solutionConverter.convertFullSolutionDtoToTrimmedSolutionDtoFlux(any())).thenReturn(Flux.fromIterable(expectedTrimmedSolutionsDto));
        ...
        StepVerifier.create(resultMono)
                .expectNextMatches(resultDto -> {
                    assertThat(resultDto.getOffset()).isEqualTo(offset);
                    assertThat(resultDto.getLimit()).isEqualTo(limit);
                    assertThat(resultDto.getCount()).isEqualTo(2); // Espera que las soluciones dentro del challengeDto sean 2
                    assertThat(resultDto.getResults().length).isEqualTo(1); // Espera que dentro de result en GenericResultDto haya 1 solo challengeDto
                    return true;
                })
                .verifyComplete();
