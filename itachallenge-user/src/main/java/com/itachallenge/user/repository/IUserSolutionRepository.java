...
public interface IUserSolutionRepository extends ReactiveMongoRepository<UserSolutionDocument, UUID> {
    ...
    Flux<UserSolutionDocument> findByUserIdAndLanguageIdAndStatus(UUID userId, UUID languageId, ChallengeStatus status);
    Flux<UserSolutionDocument> findByUserIdAndLanguageIdAndBookmarked(UUID userId, UUID languageId, boolean isBookmarked);
}
