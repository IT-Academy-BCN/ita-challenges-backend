...
public class ConverterDocumentToDto {
    ...
    public Flux<UserSolutionDto> fromUserSolutionDocumentToUserSolutionDto(UserSolutionDocument document) {
        return Flux.just(UserSolutionDto.builder()
                ...
                .solutionText(document.getSolutionDocument().getFirst().getSolutionText())
                .build());
    }

    public Flux<UserChallengeDto> fromUserSolutionDocumentToUserChallengeDto(Flux<UserSolutionDocument> documentFlux) {
        return documentFlux.map(doc ->
                UserChallengeDto.builder()
                        .uuidChallenge(doc.getChallengeId().toString())
                        .score(doc.getScore())
                        .build());
    }
