...
output_json() {
  jq -n \
   ...
   rm -f /data/input/*"${SOLUTION_ID}"* /data/input/*"${clean_solution_id}"*
}
...
if [ $? -ne 0 ]; then
  sandbox_exceptions="Failed to update class name in $clean_javaFile."
  output_json
  exit 1
fi
...
while IFS='=' read -r key value; do
  ...
  if [ $? -ne 0 ]; then
    ...
    error_list="${error_list}Execution failed for key $key. "
    continue
  fi
  ...
  if [ "$resultado" != "$value" ]; then
    ...
    error_list="${error_list}Expected $value but got $resultado for key $key. "
  fi
done < "$parametersFile"

user_code_errors="$error_list"
